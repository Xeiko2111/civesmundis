  ...
  return (
    <section className="relative overflow-hidden bg-ink-950 py-28 text-white md:py-40">
      ...
      <div className="container-page mt-14">
        {/* Contenedor relativo SIN overflow-hidden: así el panel flotante de escritorio
            puede salirse del recuadro del mapa sin quedar recortado (antes esto hacía
            que el botón de cerrar quedara inaccesible y solo se pudiera "cerrar"
            pulsando otro punto). El recorte de la imagen vive en el div de dentro. */}
        <div className="relative">
          ...
          {/* Panel flotante: solo en pantallas sm+ (en móvil el mapa es demasiado bajo
              para que un popover posicionado por punto quepa sin cortarse). */}
          {activeHotspot && active && (
            <div
              ...
              className="absolute z-20 hidden w-[280px] max-w-[80vw] border border-white/15 bg-ink-950/95 p-5 shadow-2xl shadow-black/50 backdrop-blur-md sm:block"
            >
              <ContinentPanel
                active={active}
        ...
        {/* Panel para móvil: fijo debajo del mapa, a todo el ancho, sin recortes. */}
        {active && (
          <div className="mt-5 border border-white/15 bg-ink-950/95 p-5 sm:hidden">
            <ContinentPanel
              active={active}
              activeCountries={activeCountries}
              filteredProjects={filteredProjects}
...
interface ContinentPanelProps {
  ...
  filteredProjects: typeof allCooperationProjects
  filteredPartners: typeof contrapartes
  onClose: () => void
  t: ReturnType<typeof useLanguage>['t']
}

function ContinentPanel({ active, activeCountries, filteredProjects, filteredPartners, onClose, t }: ContinentPanelProps) {
  return (
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        <button onClick={onClose} aria-label="Cerrar" className="shrink-0 text-white/40 hover:text-white">
          <X size={16} />
        </button>
      </div>
